...
该示例通过逆向映射查找目标像素在原图中的位置，并使用双线性插值计算像素值。代码中的函数说明包含参数、返回值和主要用途，同时标注了 `math.floor()`、`math.ceil()`、`math.radians()`、`math.sin()` 与 `math.cos()` 的功能，方便结合代码理解几何变换过程。
