...
| Canny 流程示意 | 手写 Canny 边缘检测结果 |
| --- | --- |
| ![Canny 流程示意](image.png) | ![手写 Canny 边缘检测结果](bird_canny.jpg) |
...
- 尺度空间的极值检测
    ...
    - 高斯差分
      - 对同一尺度下不同高斯滤波的图像进行差分
      - 经过高斯差分得到的图像更能表示图像的特征，无用成分进一步缩减，图像的边缘、轮廓进一步突显
    - 局部极值
      - SIFT算法选择高斯差分金字塔中的极值点作为特征关键点
- 关键点的精确定位
  - 对比测试：解决对比度较低的极值点，对所有极值点进行二阶泰勒展开，如果结果值太小就剔除关键点
  -  边缘测试：使用Hessian矩阵识别具有高边缘度但对少量噪声没有鲁棒性的关键点。
     - Hessian矩阵两个特征值代表两个主方向，边缘点更偏向于其中一个方向，所以特征值比值大的要删去；特征值异号（Hessian矩阵行列式＜0）说明是鞍点，不稳定要删去。
- 关键点主方向分配
...
| 原图 | SIFT 特征点提取结果 |
| --- | --- |
| ![SIFT 特征提取原图](planet.jpg) | ![SIFT 特征点提取结果](planet_sift.jpg) |
